<!-- 
  author: zkz
  @createTime: 2024-09-06 10:41:20
  @Description: 消息管理页面
 -->
<template>
  <div>
      ...
      <div class="table-top">
        ...
        <div class="table-header">
          ...
          <div class="button-wrapped">
            <el-button type="primary" @click="addAdmin(3)"
              >添加消息管理</el-button
            >
          </div>
        </div>
...
const item = reactive({
  first: "用户管理",
  second: "消息管理员",
  imgIcon: import.meta.env.VITE_APP_SVGURL + "/assets/svg/notice.svg",
});
// 搜索关键词
// 搜索关键词
const searchInput = ref<number>();
const searchAdmin = async () => {
  tableData.value = await searchMessageAdmin(searchInput.value);
};
// 表格数据
const tableData = ref([]);
// 添加管理员
...
const currentChange = async (value: number) => {
  const res = await returnListData(value, "消息管理员");
  tableData.value = res;
