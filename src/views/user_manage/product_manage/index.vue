<!-- 
  author: zkz
  @createTime: 2024-09-05 16:47:11
  @Description: 产品管理员页面
 -->
<template>
  <div>
      ...
      <div class="table-top">
        ...
        <div class="table-header">
          ...
          <div class="button-wrapped">
            <el-button type="primary" @click="addAdmin(1)"
              >添加产品管理</el-button
            >
          </div>
        </div>
              ...
              <template #default="scope">
                <div>
                  ...
                  <el-button
                    plain
                    type="danger"
                    @click="delOption(scope.row.id, scope.row.name)"
                    >删除</el-button
                  >
                </div>
              </template>
...
const item = reactive({
  first: "用户管理",
  second: "产品管理员",
  imgIcon: import.meta.env.VITE_APP_SVGURL + "/assets/svg/product.svg",
});
...
const searchAdmin = async () => {
  tableData.value = await searchProductAdmin(searchInput.value);
};
...
const delOption = (id, name) => {
  ElMessageBox.confirm("是否将该条数据降级为用户管理员?", "警告", {
    ...
    .then(async () => {
      ...
      tracking("删除", localStorage.getItem("name"), name, 3);
    })
    .catch(() => {
      ElMessage({
        type: "info",
...
const getFirstPageList = async () => {
  const res = await returnListData(paginationData.currentPage, "产品管理员");
  tableData.value = res;
};
getFirstPageList();
...
const currentChange = async (value: number) => {
  const res = await returnListData(value, "产品管理员");
  tableData.value = res;
