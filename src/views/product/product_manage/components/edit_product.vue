<!-- /* 
* @Author: zkz    
* @CreateTime: 2025-02-25 11:25:19
* @Description: 修改产品入库信息
*/ -->
<template>
  <el-dialog
...
import { editProductShow, editProduct } from "@/api/product.js";
import { getProduct } from "@/api/setting.js";
bus.on("editProductId", async (id: number) => {
  formData.id = id;
  // 数据回显
  const res = await editProductShow(formData.id);
  formData.product_id = res.product_id;
  formData.product_name = res.product_name;
  formData.product_category = res.product_category;
  formData.product_unit = res.product_unit;
  formData.product_inwarehouse_number = res.product_inwarehouse_number;
  formData.product_single_price = res.product_single_price;
  formData.product_create_person = res.product_create_person;
  formData.in_memo = res.in_memo;
});
// 弹框开关
